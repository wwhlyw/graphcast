...
class GraphCast:
    ...
    def train(self):
        ...
        for epoch in range(self.train_config['phase1_epoch']):
            ...
            self.write_to_file(folder_path='/home/wwh/graphcast/train2d', file_name='train.txt', content=f'epoch {epoch}:{train_loss}\n')
            self.valid()         
    ...
    def train_one_epoch_1or2_phase(self, train_loader, epoch): 
        ...
        for i, (input, label, input_forcings, label_forcings) in enumerate(train_loader):
            ...
            constant = self.grid_pos.unsqueeze(dim=0).expand([input.shape[0], -1, -1]).to(self.device)
    ...
    def valid(self, steps=1):
        ...
        vars = self.model_config['variables']
        self.model.eval()
        vars_records = defaultdict(lambda: torch.tensor(0))
        with torch.no_grad():
            for i, (input, label, input_forcings, label_forcings) in enumerate(valid_loader):
                ...
                per_variable_level_mean = self.per_variable_level_mean.unsqueeze(0).unsqueeze(0).repeat([1, 1, steps])
                per_variable_level_std = self.per_variable_level_std.unsqueeze(0).unsqueeze(0).repeat([1, 1, steps])
                label = (label * per_variable_level_std + per_variable_level_mean).permute(2, 0, 1)
                # [B, N, F * T] -> [F * T, B, N]
                tmp = torch.cat(predicts, axis=-1).permute(2, 0, 1)
                
                for j in range(steps):
                    for (k, variable) in enumerate(self.variables_list):
                        MSE = torch.mean((tmp[j*24+k] - label[j*24+k]) ** 2)
                        vars_records[variable + '_' + str(j)] = torch.sqrt((vars_records[variable + '_' + str(j)] ** 2 + MSE) / 2.) 

            for i in range(steps):
                for variable in self.variables_list:
                    folder_path = '/home/wwh/graphcast/predict/' + str(i)
                    file_name = variable + '.txt'
                    RMSE = vars_records[variable + '_' + str(i)]
                    content = f'rmse:{RMSE}\n'
                    self.write_to_file(folder_path, file_name, content)
            mse = []
            for rmse in vars_records.values():
                mse.append(rmse ** 2)
            mse = torch.tensor(mse)
            RMSE = torch.sqrt(torch.mean(mse))
            folder_path = '/home/wwh/graphcast/predict/'
            file_name = 'total.txt'
            content = f'rmse:{RMSE}\n'
            self.write_to_file(folder_path, file_name, content)
    ...
    def test(self, steps=1):
        valid_loader = self._init_data('valid', output_timestamps=steps)
        vars = self.model_config['variables']
        self.model = torch.load('/home/wwh/graphcast/checkpoints/30.pth').to(self.device)
        self.model.eval()
        vars_records = defaultdict(lambda: torch.tensor(0))
        with torch.no_grad():
            for i, (input, label, input_forcings, label_forcings) in enumerate(valid_loader):
    ...
    def _init_grid2mesh_graph(self):
        # print(len(self._finest_mesh.vertices))
        # 33025 mesh nodes
        ...
        self.grid_constant_feats = senders_node_features.to(self.device)
        self.mesh_node_feats = torch.tensor(self.mesh_pos, dtype=torch.float32).to(self.device)
    ...
    def _init_mesh2mesh_graph(self):
        # print('init mesh2mesh')
        merged_mesh = icosahedral_mesh.merge_meshes(self._meshes)
        senders, receivers = grid_mesh_connectivity.mesh2mesh_edges_indices_2d(merged_mesh.faces)
        # print(len(senders))
        # 263160
        edge_features = features.get_homogeneous_graph_spatial_features_2d(
            meshes_pos = merged_mesh.vertices,
            senders = senders,
            receivers = receivers,
        )
